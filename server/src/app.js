...
function buildInboxNotificationBaseWhere(userId, recentCutoff) {
  return {
    userId,
    createdAt: {
      gte: recentCutoff,
...
function buildInboxNotificationOrderBy() {
  return [{ createdAt: "desc" }, { id: "desc" }];
}

async function loadInboxSections(userId, recentCutoff, now) {
  const baseWhere = buildInboxNotificationBaseWhere(userId, recentCutoff);
  const select = buildInboxNotificationSelect();
  const orderBy = buildInboxNotificationOrderBy();

  const [needsConfirmation, declinedByYou, updates] = await Promise.all([
